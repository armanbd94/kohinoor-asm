...
class LoginController extends Controller
{
    ...
    protected function authenticated(Request $request, $user)
    {
        if($user->status == 2){
            ...
            $menus = Module::with('module_asm')->doesntHave('parent')
                ->orderBy('order','asc')
                ->with('children')
            ...
            ->where('modules.menu_id',2)
            ->whereHas('permission_asm', function($q) use ($asm_id){
                $q->where('asm_id',$asm_id);
            })
            ->select('slug')
            ->get();

            $this->user_menu = $menus; //permitted module list

            $this->user_permission = $permissions; //permitted method list
