                ...
                <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                    <form id="journal-voucher-form" method="post">
                        ...
                        <div class="row">
                                ...
                                <div class="col-md-12">
                                    <table class="table table-bordered" id="debit-voucher-table">
                                        ...
                                        <tbody>
                                            ...
                                            @endphp     
                                            @if (!$voucher->isEmpty())
                                                @foreach ($voucher as $key => $journalv)
                                                @php
                                                    $total_debit += $journalv->debit;
                                                    $total_credit += $journalv->credit;
                                                @endphp
                                                <tr>
                                                    <td>
                                                            ...
                                                            @foreach ($transactional_accounts as $coa)
                                                                @if(!empty($coa->customer_id) || !empty($coa->bank_id) || !empty($coa->mobile_bank_id))
                                                                    @if($coa->customer_id)
                                                                            ...
                                                                            <option value="{{ $coa->id }}" {{ ($journalv->chart_of_account_id == $coa->id) ? 'selected' : '' }}>{{ $coa->name }} [Customer]</option>
                                                                            @endif    
                                                                    @elseif($coa->bank_id)
                                                                        @if($coa->bank->warehouse_id == $warehouse_id)
                                                                        <option value="{{ $coa->id }}" {{ ($journalv->chart_of_account_id == $coa->id) ? 'selected' : '' }}>{{ $coa->name }}</option>
                                                                        @endif
                                                    ...
                                                    <td>
                                                        <input type="text" class="form-control text-right debit_amount" value="{{ $journalv->debit }}" onkeyup="calculate_total(1)" name="journal_account[{{ $key + 1 }}][debit_amount]" id="journal_account_{{ $key + 1 }}_debit_amount" placeholder="0.00">
                                                    </td>
                                                    <td>
                                                        <input type="text" class="form-control text-right credit_amount" value="{{ $journalv->credit }}" onkeyup="calculate_total(2)" name="journal_account[{{ $key + 1 }}][credit_amount]" id="journal_account_{{ $key + 1 }}_credit_amount" placeholder="0.00">
                                                    </td>
                                                    @if ($key == 0)
                                                        <td></td>
                                                    @else
                                ...
                                <x-form.textarea labelName="Remarks" name="remarks"  value="{{ $voucher[0]->description }}" col="col-md-12"/>
