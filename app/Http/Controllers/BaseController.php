...
{
    protected $model;
    protected $admin_base_path = 'http://kohinoor.test/';
    ...
    protected function table_image($path,$image,$alt_text,$gender=null)
    {
        ...
        {
            return "<img src='".$this->admin_base_path."storage/".$path.$image."' alt='".$alt_text."' style='width:50px;'/>";
        }else{
            if($gender){
                    return "<img src='".$this->admin_base_path."images/".($gender == 1 ? 'male' : 'female').".svg"."' alt='Default Image' style='width:50px;'/>";
            }else{
                return "<img src='".$this->admin_base_path."images/default.svg"."' alt='Default Image' style='width:50px;'/>";
            }
            
        }
    }

    protected function image($path,$image,$alt_text)
    {
        if(!empty($path) && !empty($image) && !empty($alt_text))
        {
            return "<img src='".$this->admin_base_path."storage/".$path.$image."' alt='".$alt_text."' style='width:50px;'/>";
