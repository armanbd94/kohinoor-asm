...
class MobileBankBookController extends BaseController
{
    ...
    public function index()
    {
        if(permission('mobile-bank-book-access')){
            ...
            $banks      = DB::table('chart_of_accounts as coa')
            ->select('coa.*','b.warehouse_id')
            ->leftJoin('mobile_banks as b','coa.mobile_bank_id','=','b.id')
            ->where([['coa.code','like','1020103%'],['coa.level',4],['coa.status',1],['b.warehouse_id',auth()->user()->warehouse->id]])
        ...
        if ($request->ajax()) {
            ...
            if($request->bank_acc_id){
                ...
                $previous_balance_data = DB::table('transactions')
                                            ...
                                            ->where([['voucher_date','<',$start_date],['chart_of_account_id',$bank_acc_id],['warehouse_id', $warehouse_id]])
